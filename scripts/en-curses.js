...
$('.newCurse').on('click', (e)=>{
    ...
    if(localStorage.getItem('curseCreateState') == 'true') return alert('Save the changes on the previous card to create a new one')
...
//delete curse
$('.curseDelete').click(e=>{
    const response = window.confirm('Are you sure you want to permanently delete this card?')
    if(response){
            ...
            success: (res)=>{
                if(res == 'ok'){
                    ...
                    if($('.newCurse').length == 3){
                        ...
                        <div class="newCurseChange">
                            ...
                            <div class="d-flex">
                                <div class="newCurseSave d-flex justify-content-center">
                                    <span class="newCurseSaveSp">Save</span>
                        ...
                        <div class='newCurseSymbols'>Printed characters: <span class='newCurseSymbolsSp'>0</span>/180</div>
                        </div>
                        `)
                    }
                }
...
$('.newCurseSaveChanges').click(e=>{
    ...
    if($(e.target.children[0]).css('display') != 'none'){
        ...
        if(text.replace(/ /g,'') == '' || date == ''){
            alert('Fill in all fields (card text, date)')
            $(e.target.children[0]).css({ 'display': 'block' })
            $(e.target.children[1]).css({ 'display': 'none' })
        }
        else{
