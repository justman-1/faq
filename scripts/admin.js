...
$('.createAdminBut').click(e=>{
    ...
    if(login.replace(/ /g,'') == '' || password.replace(/ /g,'') == '') return alert('Заполните все поля(логин, пароль)')
...
function deleteAdmin(e){
    ...
    let response = window.confirm('Вы точно хотите безвозвратно удалить учетную запись администратора с ником ' + login)
    if(response){
